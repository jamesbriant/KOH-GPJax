...
def test_koh_dataset_attributes(koh_dataset_instance, field_data_input, sim_data_input):
    ...
    assert koh_dataset_instance.num_calib_params == 2  # x_sim.shape[1] - x_field.shape[1]
...
def test_koh_dataset_X_method_invalid_theta_ndim(koh_dataset_instance):
    ...
    with pytest.raises(ValueError, match="Parameter theta must be a 2D array. Got theta.ndim=1"):
...
def test_koh_dataset_X_method_invalid_theta_shape(koh_dataset_instance):
    ...
    with pytest.raises(ValueError, match=r"Parameter theta must have shape \(2, 1\) OR \(1, 2\). Got theta.shape=\(1, 3\)"):
        koh_dataset_instance.X(theta_invalid_shape_1_3)

    # This theta is (3,1)
    theta_invalid_shape_3_1 = jnp.array([[1.5], [2.5], [3.5]])
    with pytest.raises(ValueError, match=r"Parameter theta must have shape \(2, 1\) OR \(1, 2\). Got theta.shape=\(3, 1\)"):
    ...
    with pytest.raises(ValueError, match=r"Parameter theta must have shape \(2, 1\) OR \(1, 2\). Got theta.shape=\(2, 2\)"):
...
def test_koh_dataset_init_incompatible_sim_field_dimensions():
    ...
    with pytest.raises(ValueError, match=r"Input dimension of simulation data \(2\) must be greater than input dimension of field data \(2\)"):
    ...
    with pytest.raises(ValueError, match=r"Input dimension of simulation data \(1\) must be greater than input dimension of field data \(2\)"):
        KOHDataset(field_dataset_ko, sim_dataset_ko_less_dim)


def test_koh_dataset_init_invalid_y_field_shape(sim_data_input): # sim_data_input is valid and can be reused
    ...
    field_dataset_ko_shape_n2 = Dataset(x_field_ko, y_field_ko_shape_n2) # Now x_field_ko (2,2) and y_field_ko_shape_n2 (2,2) match rows
    # KOHDataset's _check_shapes raises:
    # "Field observations must have shape (n, 1). Got shape=(2, 2)." for field_dataset.y.shape[1] != 1
    with pytest.raises(ValueError, match=r"Field observations must have shape \(n, 1\). Got shape=\(2, 2\)."):
         KOHDataset(field_dataset_ko_shape_n2, sim_data_input)


def test_koh_dataset_init_invalid_y_sim_shape(field_data_input): # field_data_input is valid and can be reused
    ...
    sim_dataset_ko_shape_n2 = Dataset(x_sim_ko, y_sim_ko_shape_n2) # x_sim_ko (2,3) and y_sim_ko_shape_n2 (2,2) match rows
    # KOHDataset's _check_shapes raises:
    # "Simulation outputs must have shape (N, 1). Got shape=(2, 2)." for sim_dataset.y.shape[1] != 1
    with pytest.raises(ValueError, match=r"Simulation outputs must have shape \(N, 1\). Got shape=\(2, 2\)."):
...
def test_koh_dataset_repr(koh_dataset_instance):
    ...
    assert "Field data = Dataset(Number of observations: 2 - Input dimension: 2)" in representation
    assert "Simulation data = Dataset(Number of observations: 2 - Input dimension: 4)" in representation
...
def test_koh_dataset_tree_flatten_unflatten(koh_dataset_instance, field_data_input, sim_data_input):
    ...
    assert new_instance.num_calib_params == koh_dataset_instance.num_calib_params # Check one derived attribute
