...
def mock_sub_kernels():
    return {
        "k_eta": RBF(),
        "k_delta": RBF(active_dims=[0]), # Example: acts only on the first dim of its input slice
...
def test_kohkernel_call_raises_not_implemented(mock_sub_kernels):
    ...
    with pytest.raises(NotImplementedError, match="It is no obvious how to compute the kernel value"):
...
def test_kohkernel_active_dims_propagation():
    ...
    k_delta_active = RBF(active_dims=[0]) # Will receive a slice, active_dims apply to that slice

    koh_kernel = KOHKernel(
        num_field_obs=2, num_sim_obs=2,
        k_eta=k_eta_active,
