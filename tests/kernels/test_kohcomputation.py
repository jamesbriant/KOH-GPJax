...
from jax import jit
from gpjax.kernels import RBF, White
...
def mock_koh_kernel_components():
    ...
    return {
        ...
        "k_epsilon_eta": White(variance=0.5), # Operates on all dims of its slice (x_sim)
    }
...
def setup_computation_test(mock_koh_kernel_components):
    ...
    X1_field = jnp.arange(n_field_obs * dim_k_eta_inputs).reshape(n_field_obs, dim_k_eta_inputs) * 0.1
    X1_sim = jnp.arange(n_sim_obs * dim_k_eta_inputs).reshape(n_sim_obs, dim_k_eta_inputs) * 0.2
    X1 = jnp.vstack((X1_field, X1_sim))

    X2_field = (jnp.arange(n_field_obs * dim_k_eta_inputs).reshape(n_field_obs, dim_k_eta_inputs) * 0.3) + 0.05
    X2_sim = (jnp.arange((M - n_field_obs) * dim_k_eta_inputs).reshape((M - n_field_obs), dim_k_eta_inputs) * 0.4) + 0.05
...
def test_kohkernelcomputation_cross_covariance_shape(setup_computation_test):
    ...
    jitted_cross_cov = jit(engine.cross_covariance, static_argnums=0)  # Mark kernel as static
...
def test_kohkernelcomputation_gram_shape(setup_computation_test):
    ...
    jitted_cross_cov_for_gram = jit(engine.cross_covariance, static_argnums=0) # JIT cross_cov
...
def test_kohkernelcomputation_cross_covariance_values(setup_computation_test):
    ...
    X1_sim = X1[n_field_obs : n_field_obs + kernel.num_sim_obs, :] # kernel.num_sim_obs defines the sim slice size

    def to_dense_if_needed(mat_op):
        return mat_op.to_dense() if hasattr(mat_op, "to_dense") else mat_op

    sigma_eta_expected = to_dense_if_needed(kernel.k_eta.cross_covariance(X1, X1))
    sigma_delta_expected = to_dense_if_needed(kernel.k_delta.cross_covariance(X1_field, X1_field))
    sigma_epsilon_expected = to_dense_if_needed(kernel.k_epsilon.cross_covariance(X1_field, X1_field))
    sigma_epsilon_eta_expected = to_dense_if_needed(kernel.k_epsilon_eta.cross_covariance(X1_sim, X1_sim))
    ...
    n_field_obs_s, n_sim_obs_s, dim_k_eta_inputs_s = s["n_field_obs"], kernel.num_sim_obs, X_generic1.shape[1]
    N_current = X_generic1.shape[0]
    M_for_error_check = N_current + 1 # Ensure M != N

    X2_field_err = (jnp.arange(n_field_obs_s * dim_k_eta_inputs_s).reshape(n_field_obs_s, dim_k_eta_inputs_s) * 0.7) + 0.01
    X2_sim_err = (jnp.arange((M_for_error_check - n_field_obs_s) * dim_k_eta_inputs_s).reshape((M_for_error_check - n_field_obs_s), dim_k_eta_inputs_s) * 0.8) + 0.01
