import pytest
from jax import jit, numpy as jnp # Removed 'config'
import numpyro.distributions as npd
from numpyro.distributions import transforms as npt # npt is used in a commented out section, but might be useful. Keeping for now.
import jax.tree_util as jtu

from kohgpjax.parameters import ModelParameters, ParameterPrior, PriorDict
...
def valid_prior_dict() -> PriorDict:
    return {
        ...
        "eta": { # This should contain 'variances', 'lengthscales', etc.
            "variances": {"kernel_var": ParameterPrior(npd.Normal(1,0.5))}, # param_idx: 2
            "lengthscales": {
        ...
        "delta": { # Similar structure to eta
            "variances": {"kernel_var": ParameterPrior(npd.Normal(0,1))},   # param_idx: 5
        ...
        "epsilon": { # Optional, not strictly checked by top-level of _check_prior_dict beyond presence
            ...
            "variances": {"obs_noise": ParameterPrior(npd.Normal(-0.5,0.1))} # param_idx: 7
        },
...
@pytest.fixture(scope="module")
def model_params(valid_prior_dict: PriorDict) -> ModelParameters:
...
def assert_trees_equal(tree1, tree2, rtol=1e-7, atol=1e-9):
    ...
    assert treedef1 == treedef2, f"Tree structures do not match: {treedef1} vs {treedef2}"
    for i, (v1, v2) in enumerate(zip(flat1, flat2)):
        assert jnp.allclose(v1, v2, rtol=rtol, atol=atol), f"Leaf {i} mismatch: {v1} vs {v2}"

# --- Tests ---

def test_model_parameters_initialization(model_params: ModelParameters, valid_prior_dict: PriorDict):
...
def test_get_log_prior_func(model_params: ModelParameters, mcmc_sample_unconstrained: list[float]):
    ...
    expected_lp = (
        ...
        flat_priors_actual_order[1].distribution.log_prob(mcmc_sample_unconstrained[1]) +
        flat_priors_actual_order[2].distribution.log_prob(mcmc_sample_unconstrained[2]) +
        flat_priors_actual_order[3].distribution.log_prob(mcmc_sample_unconstrained[3]) +
        flat_priors_actual_order[4].distribution.log_prob(mcmc_sample_unconstrained[4]) +
        flat_priors_actual_order[5].distribution.log_prob(mcmc_sample_unconstrained[5]) +
        flat_priors_actual_order[6].distribution.log_prob(mcmc_sample_unconstrained[6]) +
        flat_priors_actual_order[7].distribution.log_prob(mcmc_sample_unconstrained[7])
    )
...
def test_unflatten_sample(model_params: ModelParameters, mcmc_sample_unconstrained: list[float]):
    ...
    expected_unflattened = {
        "delta": {
            "lengthscales": {"ls_x": mcmc_sample_unconstrained[0]}, # sorted: lengthscales before variances
        ...
        "eta": {
            "lengthscales": { # sorted: ls_theta before ls_x if keys are "ls_theta", "ls_x"
                ...
                "ls_theta": mcmc_sample_unconstrained[4], # ls_theta is dict key, ls_x is dict key.
                "ls_x": mcmc_sample_unconstrained[3],     # JAX sorts these keys: ls_theta, ls_x. So sample[3] is for ls_theta, sample[4] for ls_x
            },
    ...
    reconstructed_tree = jtu.tree_unflatten(model_params.priors_tree, mcmc_sample_unconstrained)
    ...
    assert_trees_equal(actual_unflattened, reconstructed_tree) # Compare against programmatically unflattened
    assert_trees_equal(jitted_actual_unflattened, reconstructed_tree)


def test_constrain_sample(model_params: ModelParameters, mcmc_sample_unconstrained: list[float]):
...
def test_constrain_and_unflatten_sample(model_params: ModelParameters, mcmc_sample_unconstrained: list[float]):
    ...
    expected_constrained_unflattened = jtu.tree_unflatten(model_params.priors_tree, mcmc_sample_unconstrained)

    actual_constrained_unflattened = constrain_unflatten_func(mcmc_sample_unconstrained)
    jitted_actual_constrained_unflattened = jitted_constrain_unflatten_func(mcmc_sample_unconstrained)

    assert_trees_equal(actual_constrained_unflattened, expected_constrained_unflattened)
    assert_trees_equal(jitted_actual_constrained_unflattened, expected_constrained_unflattened)
...
def test_model_parameters_empty_prior_dict_fails():
    ...
    empty_prior_dict: PriorDict = {}
    with pytest.raises(ValueError, match="prior_dict keys must contain \\['thetas', 'eta', 'delta'\\]"):
...
def test_model_parameters_alternative_priors_valid_structure():
    ...
    alt_prior_dict: PriorDict = {
        "thetas": {"calib": ParameterPrior(npd.Normal(0,1))},
        "eta": {
            "variances": {"k_var": ParameterPrior(npd.Exponential(1.0))}, # Non-Identity
    ...
    assert params.n_params == 5 # 1 (thetas) + 1 (eta/var) + 1 (eta/ls) + 1 (delta/var) + 1 (delta/ls)
    ...
    for i, prior_item in enumerate(params.priors_flat):
        ...
        if isinstance(prior_item.distribution, (npd.Exponential, npd.HalfNormal, npd.LogNormal)):
            assert constrained_val > 0, f"Prior {prior_item} gave non-positive value {constrained_val}"
        elif isinstance(prior_item.distribution, npd.Normal):
            assert jnp.isclose(constrained_val, unconstrained_val), f"Prior {prior_item} (Normal) did not return identity: {constrained_val} vs {unconstrained_val}"
...
def test_model_parameters_pytree_registration(model_params: ModelParameters):
    ...
    reconstructed_priors = jtu.tree_unflatten(model_params.priors_tree, model_params.priors_flat)

    original_flat_priors_info = [(type(p.distribution), p.distribution.arg_constraints) for p in model_params.priors_flat]
    reconstructed_flat, _ = jtu.tree_flatten(reconstructed_priors) # reconstructed_priors is already the dict
    reconstructed_flat_priors = jtu.tree_leaves(reconstructed_priors, is_leaf=lambda x: isinstance(x, ParameterPrior))

    reconstructed_flat_priors_info = [(type(p.distribution), p.distribution.arg_constraints) for p in reconstructed_flat_priors]

    # Sort them before comparing as flatten order might differ if dicts were created differently
    assert sorted(original_flat_priors_info, key=str) == sorted(reconstructed_flat_priors_info, key=str)
...
def test_parameter_prior_non_identity_bijector():
    ...
    expected_log_prob = npd.Exponential(rate=1.0).log_prob(jnp.exp(unconstrained_val)) + unconstrained_val
