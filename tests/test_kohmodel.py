...
class MinimalKOHModel(KOHModel):
    ...
    def k_delta(self, params_constrained: dict) -> gpx.kernels.AbstractKernel:
        ...
        return gpx.kernels.RBF(lengthscale=ls, variance=var, active_dims=list(range(self._kohdataset_cached_for_test.num_variable_params)))
...
def simple_prior_dict(koh_dataset_fixture) -> PriorDict: # Depends on koh_dataset_fixture for num_calib_params
    num_calib = koh_dataset_fixture.num_calib_params
    thetas_priors = {f"theta_{i}": ParameterPrior(npd.Normal(0,1)) for i in range(num_calib)}

    return {
        "thetas": thetas_priors,
        "eta": {
            "variances": {"var": ParameterPrior(npd.LogNormal(0,1))}, # For RBF variance
            "lengthscales": {"ls": ParameterPrior(npd.LogNormal(0,1))}  # For RBF lengthscale
        },
        "delta": {
            "variances": {"var": ParameterPrior(npd.LogNormal(0,1))},
            "lengthscales": {"ls": ParameterPrior(npd.LogNormal(0,1))}
        },
        "epsilon": {"variances": {"obs_noise": ParameterPrior(npd.HalfNormal(1.0))}}, # For k_epsilon if obs_stddev is None
    }

@pytest.fixture(scope="module")
def model_parameters_fixture(simple_prior_dict: PriorDict) -> ModelParameters:
...
def koh_dataset_fixture() -> KOHDataset:
    ...
    x_sim = jnp.array([[1.0, 0.1], [2.0, 0.2]]) # 2 obs, 1 var_param, 1 "design" param for sim (becomes calib)
...
@pytest.fixture(scope="module")
def gpjax_params_fixture(model_parameters_fixture: ModelParameters) -> dict:
    ...
    return model_parameters_fixture.constrain_and_unflatten_sample(dummy_flat_unconstrained)

@pytest.fixture
def minimal_koh_model_fixture(model_parameters_fixture: ModelParameters, koh_dataset_fixture: KOHDataset) -> MinimalKOHModel:
...
@pytest.fixture
def minimal_koh_model_static_obs_fixture(model_parameters_fixture: ModelParameters, koh_dataset_fixture: KOHDataset) -> MinimalKOHModel:
    return MinimalKOHModel(
        model_parameters=model_parameters_fixture,
        kohdataset=koh_dataset_fixture,
...
def test_kohmodel_initialization(minimal_koh_model_fixture: MinimalKOHModel, model_parameters_fixture, koh_dataset_fixture):
    ...
    model_static = MinimalKOHModel(model_parameters_fixture, koh_dataset_fixture, obs_stddev=Static(jnp.array([0.5])))
...
def test_gp_prior_mean_function(minimal_koh_model_fixture: MinimalKOHModel):
    assert isinstance(minimal_koh_model_fixture.GP_prior_mean_function(), gpx.mean_functions.Zero)

def test_k_epsilon(minimal_koh_model_fixture: MinimalKOHModel, minimal_koh_model_static_obs_fixture: MinimalKOHModel, gpjax_params_fixture: dict, koh_dataset_fixture: KOHDataset):
    ...
    assert k_eps_dynamic.active_dims == list(range(koh_dataset_fixture.num_variable_params))
    ...
    assert k_eps_static.active_dims == list(range(koh_dataset_fixture.num_variable_params))

def test_gp_kernel(minimal_koh_model_fixture: MinimalKOHModel, gpjax_params_fixture: dict, koh_dataset_fixture: KOHDataset):
    model = minimal_koh_model_fixture
    ...
    assert jnp.isclose(kernel.k_epsilon_eta.variance.value, 0.0) # k_epsilon_eta is White(0.0)

def test_likelihood(minimal_koh_model_fixture: MinimalKOHModel, gpjax_params_fixture: dict, koh_dataset_fixture: KOHDataset):
    model = minimal_koh_model_fixture
...
def test_gp_prior(minimal_koh_model_fixture: MinimalKOHModel, gpjax_params_fixture: dict):
    model = minimal_koh_model_fixture
...
def test_gp_posterior(minimal_koh_model_fixture: MinimalKOHModel, gpjax_params_fixture: dict):
    model = minimal_koh_model_fixture
    posterior = model.GP_posterior(gpjax_params_fixture)
    assert isinstance(posterior, KOHPosterior) # or gpx.gps.Posterior if KOHPosterior is a direct alias/subclass
...
def test_get_koh_neg_log_pos_dens_func(minimal_koh_model_fixture: MinimalKOHModel, model_parameters_fixture: ModelParameters):
    model = minimal_koh_model_fixture
