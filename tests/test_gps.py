...
def test_koh_posterior_predict_zeta_type(
    ...
    jitted_predict_zeta_dynamic_arg = jit(
        posterior.predict_zeta
    )  # include_observation_noise is not static here
    ...
    with pytest.raises(jax_errors.TracerBoolConversionError):
        jitted_predict_zeta_dynamic_arg(
            test_input_points_fixture,
            train_data=mock_dataset_fixture,
            include_observation_noise=False,
        )
