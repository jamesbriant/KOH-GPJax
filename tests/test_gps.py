import pytest
from jax import jit, numpy as jnp, tree_util as jtu, errors as jax_errors # Removed 'config'
import gpjax as gpx
...
def mock_koh_kernel_for_prior() -> KOHKernel:
    ...
    return KOHKernel(
        ...
        k_epsilon_eta=gpx.kernels.White(variance=0.01) # Small non-zero for distinctness
    )
...
def mock_likelihood_fixture() -> gpx.likelihoods.Gaussian:
    ...
    return gpx.likelihoods.Gaussian(num_datapoints=Static(jnp.array(4))) # Example: 2 field + 2 sim
...
def test_construct_posterior_returns_conjugateposterior(
    ...
    posterior = construct_posterior(mock_standard_prior_fixture, mock_likelihood_fixture)
...
def test_koh_posterior_predict_raises_not_implemented(
    ...
    posterior = construct_posterior(mock_prior_fixture, mock_likelihood_fixture)
    with pytest.raises(NotImplementedError, match="Use predict_eta\\(\\), predict_zeta\\(\\) or predict_obs\\(\\) methods instead\\."):
...
def test_koh_posterior_predict_eta_type(
    ...
    posterior = construct_posterior(mock_prior_fixture, mock_likelihood_fixture)
    prediction = posterior.predict_eta(test_input_points_fixture, train_data=mock_dataset_fixture)
    assert isinstance(prediction, GaussianDistribution)
    # Check shapes based on test_input_points_fixture
    assert prediction.mean.shape == (test_input_points_fixture.shape[0],) # Mean is 1D if output dim is 1
    assert prediction.covariance().shape == (test_input_points_fixture.shape[0], test_input_points_fixture.shape[0])
...
def test_koh_posterior_predict_zeta_type(
    ...
    prediction_false = posterior.predict_zeta(test_input_points_fixture, train_data=mock_dataset_fixture, include_observation_noise=False)
    assert isinstance(prediction_false, GaussianDistribution)
    assert prediction_false.mean.shape == (test_input_points_fixture.shape[0],) # Mean is 1D
    assert prediction_false.covariance().shape == (test_input_points_fixture.shape[0], test_input_points_fixture.shape[0])

    # Test with include_observation_noise = True
    prediction_true = posterior.predict_zeta(test_input_points_fixture, train_data=mock_dataset_fixture, include_observation_noise=True)
    assert isinstance(prediction_true, GaussianDistribution)
    assert prediction_true.mean.shape == (test_input_points_fixture.shape[0],) # Mean is 1D
    assert prediction_true.covariance().shape == (test_input_points_fixture.shape[0], test_input_points_fixture.shape[0])

    # Check JIT compilability
    # JIT with static include_observation_noise works for both False and True
    jitted_predict_zeta_static = jit(posterior.predict_zeta, static_argnames="include_observation_noise")

    prediction_jit_false = jitted_predict_zeta_static(test_input_points_fixture, train_data=mock_dataset_fixture, include_observation_noise=False)
    assert isinstance(prediction_jit_false, GaussianDistribution)

    prediction_jit_true = jitted_predict_zeta_static(test_input_points_fixture, train_data=mock_dataset_fixture, include_observation_noise=True)
    assert isinstance(prediction_jit_true, GaussianDistribution)

    # The source comment "This cannot be jitted. #TODO: Find a way to make this jittable."
    # likely refers to the case where `include_observation_noise` is a traced value (not static).
    # If we try to JIT without making it static, JAX would try to trace the Python `if` statement.
    # This would lead to a ConcretizationTypeError if `include_observation_noise` is changed after compilation.
    # Testing this specific JAX error is more precise.
    jitted_predict_zeta_dynamic_arg = jit(posterior.predict_zeta) # include_observation_noise is not static here

    # The TracerBoolConversionError occurs when a traced value (like a boolean arg not marked static)
    # is used in Python control flow (an `if` statement).
    with pytest.raises(jax_errors.TracerBoolConversionError):
        jitted_predict_zeta_dynamic_arg(test_input_points_fixture, train_data=mock_dataset_fixture, include_observation_noise=False)

    with pytest.raises(jax_errors.TracerBoolConversionError):
        jitted_predict_zeta_dynamic_arg(test_input_points_fixture, train_data=mock_dataset_fixture, include_observation_noise=True)
...
def test_koh_posterior_predict_obs_type(
    ...
    posterior = construct_posterior(mock_prior_fixture, mock_likelihood_fixture)
    prediction = posterior.predict_obs(test_input_points_fixture, train_data=mock_dataset_fixture)
    assert isinstance(prediction, GaussianDistribution)
...
def test_koh_posterior_pytree(
    ...
    assert reconstructed_posterior.prior.kernel.num_field_obs == posterior.prior.kernel.num_field_obs
