...
def _check_prior_dict(prior_dict: ModelParameterPriorDict) -> None:
    ...
    required_keys = [
        ...
        "eta",
        "delta",
    ]  # 'epsilon' and 'epsilon_eta' are optional
    ...
    for key, param_prior_dict in prior_dict.items():
        if key == "thetas":
            if not isinstance(param_prior_dict, dict):
                raise ValueError(
                    f"prior_dict['{key}'] must be a dictionary of ParameterPrior instances."
                )
            for sub_param_name, sub_param_item in param_prior_dict.items():
                if not isinstance(sub_param_item, ParameterPrior):
                    raise ValueError(
                        f"prior_dict['thetas']['{sub_param_name}'] must be a ParameterPrior instance."
                    )
            ...
            for sub_param_type, sub_param_dict in param_prior_dict.items():
                if not isinstance(sub_param_dict, dict):
                    raise ValueError(
                        f"prior_dict['{key}']['{sub_param_type}'] must be a dictionary of ParameterPrior instances."
                    )

                for sub_param_type_name, param_prior in sub_param_dict.items():
                    if not isinstance(param_prior, ParameterPrior):
                        raise ValueError(
                            f"prior_dict['{key}']['{sub_param_type}']['{sub_param_type_name}'] must be a ParameterPrior instance."
                        )
